...
campos <- ReadNetCDF(here("analysis/data/derived_data/reanalysis/geopotential.nc")) %>% 
  ...
  .[, ":="(msl = ReadNetCDF(here("analysis/data/derived_data/reanalysis/pressure-pw.nc"), vars = "msl", out = "vector")[[1]],
           ...
           tcw = ReadNetCDF(here("analysis/data/derived_data/reanalysis/cape-pw.nc"), vars = "tcw", out = "vector")[[1]],
           tcwv = ReadNetCDF(here("analysis/data/derived_data/reanalysis/cape-pw.nc"), vars = "tcwv", out = "vector")[[1]],
           td = ReadNetCDF(here("analysis/data/derived_data/reanalysis/td2m.nc"), vars = "d2m", out = "vector")[[1]])]

# campos <-  ReadNetCDF(here("analysis/data/derived_data/reanalysis/cape-pw.nc"), vars = "tcw")
...
campos %>% 
  ...
  # label_size = 2, label.placer = label_placer_fraction(frac = 0.75)) +
...
A relaxation-to-prior spread inflation [@whitaker2012] is applied with an inflation parameter $\alpha=0.9$ to mitigate the impact of sampling errors and to consider model errors not accounted for by the multi-model ensemble approach.
...
where $x_o$ is a assimilated observation and its error $\sigma_o$, the ensemble mean of the analysis in observational space $m$, and the standard deviation $\sigma$ of the ensemble. 
The average of $RCRV$ computed over all the realizations or analysis cycles, represents the bias of the ensemble mean with respect to the observations normalized by the estimated uncertainty:
...
where $M$ is the ensemble size. A consistent system will have no bias ($mean RCRV = 0$) and a standard deviation equal to 1 ($sd RCRV = 1$). If the ensemble has a positive bias, $mean RCRV$ will be positive, on the opposite, if the ensemble has a negative bias, $mean RCRV$ will be negative. A $sd RCRV > 1$ indicates that the difference between the observations and the ensemble mean is greater than combined uncertainty of the observations and the model, hence, the ensemble is underdispersive. On the contrary, an $sd RCRV < 1$ will indicate that the ensemble is overdispersive [@candille2007]. 
...
```{r era5, fig.cap="(ref:era5-cap)", fig.env="figure*", fig.height=7, fig.align="center"}
era <- fread("~/datosmunin3/DATA_EXP/ERA5/perfiles_ana_era5.csv")

files <- Sys.glob(here::here("/home/paola.corrales/datosmunin3/EXP//derived_data/perfiles_ana_E[2569]_new.csv"))

exp <- purrr::map(files, function(f) {
  exp <- unglue(basename(f), "perfiles_{run}_{exp}_new.csv")
  fread(f) %>% 
    .[, exp := exp[[1]][["exp"]]]
