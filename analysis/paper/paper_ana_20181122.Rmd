...
(ref:case) ERA5 Reanalysis of pressure (hPa) at sea level (black contours), 1000-500 hPa thickness (red dashed contours) and convective available potential energy (shaded) for 00 and 12 UTC Nov 22 and 00 UTC Nov 23. 
...
fisica %>% 
  ...
  kable(booktabs = TRUE, caption = "Generation of the 60-member RRR multi-physics ensemble as a combination of Cumulus and PBL parameterizations, and GEFS members for boundary conditions.", 
        align = "cccc") %>% 
...
To evaluate the statistical consistency of the uncertainty quantification in the ensemble system we use the Reduced Centered Random Variable (RCRV, @candille2007) which is defined as:
...
where $x_o$ is the assimilated observation and its error $\sigma_o$, the ensemble mean of the analysis in observational space $m$, and the standard deviation $\sigma$ of the ensemble.  The $RCRV$ is the ratio between the distance between the observations and the forecast and its expected standard deviation assuming the statistical independence between the forecast error (estimated from the ensemble spread) and the observation error. The average of $RCRV$ computed over all the realizations represents the bias of the ensemble mean with respect to the observations normalized by the estimated uncertainty:
...
For the comparison of the first-guess precipitation with the IMERG precipitation estimates, we compute the Fractions Skill Score (FSS, @roberts2008) for different neighborhood length scales and thresholds: 
...
```
We also compared ensemble mean analyses to ERA5 reanalyses [@era5pressure], which suports Figures
\@ref(fig:TQ-diff) and \@ref(fig:UV-diff).  Specifically, Figure \@ref(fig:era5)a shows a warm bias in low levels (i.e. CONV is warmer than ERA5) that decreases in Figure \@ref(fig:era5)b when we assimilate the AWS observations. In the same direction, Figure \@ref(fig:TQ-diff)a shows a negative difference between AWS and CONV meaning that the AWS observations are cooling the low levels. Comparing ERA5 with RAD (Figure \@ref(fig:era5)d), there is a see a small increase in the warm bias, associated with the warming produced by the radiance observations (Figure \@ref(fig:TQ-diff)c). A similar effect can be observed for specific humidity, AWS observations parcialy corrects the dry bias present in Figure \@ref(fig:era5)e and the assimilation of radiance observations reduces this effect. The impact on the wind components is minor and only the meridional wind is included in Figure \@ref(fig:era5). In this case Figures \@ref(fig:era5)i-l show that the radiance observations are mainly responsible for the positive impact observed in the analysis. Moreover, these adjustments due to assimilating radiance and AWS observations lead to ensemble mean analyses closer to ERA5 reanalyses.
...
```{r era5, fig.cap="(ref:era5-cap)", fig.env="figure*", fig.width=7, fig.height=6, fig.align="center"}
...
era[exp, on = c("lev", "date")] %>% 
  ...
  labs(fill = latex2exp::TeX("Specific \nhumidity ($g$ $Kg^{-1}$)"),
       x = NULL,
       y = "Pressure (hPa)") +
  tag_facets(position = list(x = 0.1, y = 0.96), tag_pool = c("e", "f", "g", "h")) +
  theme_minimal(base_size = 8) +
  theme(legend.position = "bottom",
        tagger.panel.tag.text = element_text(size = 8),
        panel.ontop = TRUE,
  ...
  era[exp, on = c("lev", "date")] %>% 
    ...
    labs(fill = latex2exp::TeX("V ($m$ $s^{-1}$)"),
         x = NULL,
         y = "Pressure (hPa)") +
    tag_facets(position = list(x = 0.1, y = 0.96), tag_pool = c("i", "j", "k", "l")) +
    theme_minimal(base_size = 8) +
    theme(legend.position = "bottom",
          tagger.panel.tag.text = element_text(size = 8),
          panel.ontop = TRUE,
...
To summarize, in this study we conclude that the assimilation of surface observations with high spatial and temporal resolution, satellite-derived winds, and clear-sky radiances from polar orbiting satellites had an overall positive impact on the development of the studied MCS and its associated precipitation. Moreover, the ensemble forecasts initialized from the analysis show promising results to predict the precipitation of severe events. In the future, we will further analyze the impacts in the independent forecasts and evaluate the implementation of other sources of observations like GPS radio occultation data and radiances from geostationary orbiting satellites like GOES-16. 
 
# Code and data availability

A version-controlled repository of the code used to create this analysis, including the code used to download the data can be found at <https://github.com/paocorrales/mesoda>. The derived data that support the findings of this study are also openly available in Zenodo at http://doi.org/10.5072/zenodo.1060834, version 0.9.0. 
