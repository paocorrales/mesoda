...
files <- Sys.glob("/home/paola.corrales/datosmunin3/EXP/E2/ANA/*/diagfiles/asim*ensmean")
...
files <- Sys.glob("/home/paola.corrales/datosmunin3/EXP/E5/ANA/*/diagfiles/asim*ensmean")[1:67]
...
files <- Sys.glob("/home/paola.corrales/datosmunin3/EXP/E6/ANA/*/diagfiles/asim*ensmean")[1:67]
...
rbind(fread(here("analysis", "data", "derived_data", "rcrv_V_perfil.csv")), 
  ...
  purrr::map(Sys.glob(here("analysis/data/derived_data/rcrv_*_perfil.csv"))[1:6], function(f) {
...
files <- Sys.glob(paste0(derived_data, "perfiles_ana_E[2,5,6,9].csv"))
...
files <- Sys.glob(paste0(derived_data, "pw/pw_ana_E[2,5,6,9]_20181122000000.nc"))
...
files <- Sys.glob(paste0(derived_data, "ppacum/E[2569]_ana*_pp_pmm.rds"))
...
readRDS(here("analysis/data/derived_data/IMERG_1h.rds")) %>% 
  .[, c("lon", "lat") := wrf_project(x, y, inverse = TRUE, round = FALSE)] %>% 
  .[x %between% c(-300000, 800000) & y %between% c(-800000, 1090000)] %>% 
...
files <- Sys.glob(here("analysis/data/derived_data/fss_6h_ana_ens_E[2569].csv"))
...
files <- Sys.glob(paste0(derived_data, "dbz/E[29]_ana_", dates, "_dbz_pmm.rds"))
...
files <- Sys.glob(paste0(derived_data, "sondeos/ANA/sondeo_E[2569]_ana*")
...
files <- Sys.glob(here("analysis/data/derived_data/fss_6h_fcst_ens_*_E[2,5,6,9]_ens.csv"))
