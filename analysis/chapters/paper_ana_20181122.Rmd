...
(ref:dominio) a) The horizontal domain used for the simulations (black box), the inner domain used for the experiment comparison (red box), the region shown in b) (light blue box) and the locations of Automatic Surface Weather Stations (ASWS, green squares) and Conventional Surface Weather Stations (CSWS, orange triangles). b) Locations of radiosonde launches during the experiment. Green dots corresponds to radiosondes launched during IOP 7, orange triangles are radiosondes launched during IOP 8, and purple squares are radiosondes launched outside the IOP missions. The topography in meters is also shown (shaded).
...
(ref:obs-horizontal) Horizontal spatial distribution of the mean available observations per analysis cycle for the a) CONV, b) ASWS c) SATWND, and d) RAD experiments calculated over 2.5$^{\circ}$ boxes.
...
(ref:TQ-diff) Difference between analysis ensemble mean experiments a) and b) ASWS-CONV, b) and e) SATWND-ASWS and c) and f) RAD-SATWND for the spatially averaged vertical profiles of temperature (a, b, and c, in $K$) and specific humidity (d, e, and f in $gkg^{-1}$) calculated over the inner domain (red box in Figure \@ref(fig:dominio)a) for each analysis cycle.
...
(ref:UV-diff)  Difference between analysis ensemble mean experiments a) and b) ASWS-CONV, b) and e) SATWND-ASWS and c) and f) RAD-SATWND for the spatially averaged vertical profiles of u wind (a, b, and c, in $ms^{-1}$) and v wind (d, e, and f in $ms^{-1}$) calculated over the inner domain (red box in Figure \@ref(fig:dominio)a) for each analysis cycle. Black contours corresponde to u wind for ASWS (a), SATWND (b) and RAD (c) and v wind for ASWS (d), SATWND (e) and RAD (f).
...
files <- Sys.glob(paste0(derived_data, "mucape/mcape_ana_E[2,5,6,9]_20181122000000.nc"))
...
files <- Sys.glob("/home/paola.corrales/datosmunin3/EXP/derived_data/ppacum/E[2569]_ana*_pp_pmm.rds")
...
(ref:fss) FSS calculated over a 6-hours moving window for 1 mm (a and c) and 25 mm (b and d) thresholds, on a 10 km (a and b) and 100 km (c and d) scales, for the first-guess of CONV (blue line), ASWS (ligh blue line), SATWND (orange line) and RAD (red line) experiments.

```{r fss, fig.cap="(ref:fss)", fig.width=3.5, fig.height=3.5, fig.pos="t"}
files <- Sys.glob("/home/paola.corrales/mesoda/analysis/data/derived_data/fss_6h_ana_ens_E[2569].csv")
...
(ref:dbz-mean) Maximum reflectivity in the column (COLMAX in $dBZ$), observed (upper row) and 1-hr forecast ensemble mean maximum reflectivity for CONV (second row) and RAD (third row) at 10 UTC (first column), 13 UTC (second column), 16 UTC (third column), and 19 UTC (fourth column) Nov 22.
...
files <- Sys.glob(paste0(derived_data, "dbz/maxdbz_ana_E[29]_", dates, ".nc"))
...
(ref:soundings) RMSE (solid line) and Bias (dashed line) of a) temperature ($K$), b) dew point temperature ($K$), c) u wind ($ms^{-1}$) and d) v wind ($ms^{-1}$) calculated by comparing each experiment with the RELAMPAGO soundings during IOP 7 and IOP 8. The blue line corresponds to CONV, the light blue line to ASWS, SATWND is represented with an orange line, and RAD with a red line.

```{r soundings, fig.cap="(ref:soundings)", fig.env="figure*", fig.pos="t", fig.align="center"}
files <- Sys.glob("/home/paola.corrales/datosmunin3/EXP/derived_data/sondeos/ANA/sondeo_E[2569]_ana*")
...
sondeos %>% 
  ...
  facet_grid(iop~variable, scales = "free_x", 
             labeller = labeller(variable = c("t" = "Temperature (K)",
