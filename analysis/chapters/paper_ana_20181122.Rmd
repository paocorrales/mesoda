...
(ref:obs-horizontal) Horizontal spatial distribution of the mean available observations per analysis cycle for the a) CONV, b) AUT c) SATWND and d) RAD experiments calculate over 2.5º boxes.
...
(ref:obs-cycle) a) Number of available observations per cycle and b) time averaged number of observations per cycle and divided into 50 hPa-depth vertical layers for the CONV (blue line), AUT (light blue line), SATWND (orange line) and RAD (red line) experiments.

```{r obs-cycle, fig.cap="(ref:obs-cycle)", fig.env="figure*"}
...
* The Satellite precipitation estimation IMERG Final Run with 0.01◦ spatial resolution and 30 minutes temporal resolution [@huffman2018] was used as a reference state to validate the skill 1-hour forecasts to represent the precipitation over the domain. 
...
```{r cycle, fig.cap="Diagram of the analysis cycles between Nov 20th, 18 UTC and Nov 23rd 12 UTC plus and spin up period of 6 hours. The zoomed section shows the assimilation for each hour that is performed within a one-hour centered window and new boundary conditions from GFSD every 6 hours. Also the two Intensive Observation Periods (IOP) from the RELAMPAGO Field Campaign are shown.", out.width="100%"}
