...
p2 <- rbind(conv, aut, satwnd) %>% 
  ...
  ggplot(aes(exp, lev.box)) +
  geom_tile(aes(fill = obs_cycle)) +
  scale_fill_viridis_c(option = "D", trans = scales::log10_trans(),
                       guide = guide_colorbar(barheight = NULL)) +
  scale_x_discrete(labels = c("CONV", "AUT", "SATWND")) +
  scale_y_reverse() +
  labs(x = NULL, y = "Pressure level (hPa)", fill = "Mean number of\nobs per cycle") +
  theme_minimal() +
  theme(aspect.ratio = 2.5/1, plot.margin = unit(c(0, 0, 0, 0), "cm"))
...
p3 <- rbind(conv, aut, satwnd) %>% 
  ...
  scale_color_manual(values = colores_exp, 
                     labels = c(E4 = "CONV", E5 = "AUT", 
                                E6 = "SATWND", E7 = "RAD")) +
  scale_date() +
  labs(y = "Mean number of obs per cycle", color = NULL, x = NULL) +
  theme_minimal() +
...
```{r}
files <- Sys.glob(here("analysis/data/derived_data/count_obs_E*"))
