...
cordillera <- list(
  ...
  scale_fill_gradient(low = "#E2E6E6", high = "#7E8E8E", guide = "none",
                      oob = scales::squish))
...
experiments <- c("E2", "E5", "E6", "E8")
#Color blind friendly but E4 y E8 parecidos en Desaturated
colores_exp <- c(E2 = "#0077BB", E5 = "#88CCEE", E6 = "#EE8733", E9 = "#CC3311") 
# colores_exp <- c(E4 = "#225555", E5 = "#CCBB44", E6 = "#CC6677", E8 = "#882255")
...
```{r}
files <- Sys.glob("/home/paola.corrales/mesoda/analysis/data/derived_data/fss_6h_fcst_ens_*_E[2,5,6,8,9]_ens.csv")
...
```{r}
files <- Sys.glob("/home/paola.corrales/datosmunin3/EXP/derived_data/ppacum/E[2569]_fcst_201811220*pp_pmm.rds")

ppacum_1h <- purrr::map(files, function(f) {
  
  meta <- unglue(basename(f), "{exp}_fcst_{inidate}_pp_pmm.rds")
  
  readRDS(f) %>% 
    .[, c("lon", "lat") := wrf_project(x, y, inverse = TRUE, round = FALSE)] %>% 
    .[, ":="(rank = NULL, 
             ens_mean = NULL,
             ini_date = ymd_h(meta[[1]][["inidate"]]))] %>% 
...
imerg <- readRDS("/home/paola.corrales/mesoda/analysis/data/derived_data/IMERG_1h.rds") %>% 
  .[, c("lon", "lat") := wrf_project(x, y, inverse = TRUE, round = FALSE)] %>% 
  .[x %between% c(-990000, 990000) & y %between% c(-1190000, 1190000)] %>% 
  .[end_date %between% c(as_datetime("20181122060000"), as_datetime("20181123120000"))] %>% 
  .[, exp := "IMERG"]
# %>% 
...
ppacum_1h[, .(mm = sum(pp_acum)/47561), by = .(exp, date, ini_date)] %>% 
  ggplot(aes(date, mm)) +
  geom_line(aes(color =  exp, linetype = factor(ini_date))) +
    scale_color_manual(values = colores_exp, labels = c(E2 = "CONV", E5 = "AUT",
                                                      E6 = "SATWND", E9 = "RAD")) +
  geom_line(data = imerg[, .(mm = sum(pp_acum)/47561), by = .(exp, end_date)], 
            aes(x = end_date, linetype = "IMERG"), color = "black") +
  labs(x = "Time", y = "Total pp [mm/grid points]", color = NULL, linetype =  NULL) +
  theme_minimal()
```
...
readRDS("/home/paola.corrales/mesoda/analysis/data/derived_data/IMERG_1h.rds") %>% 
  ...
  scale_fill_distiller(palette = "YlGnBu",
                       ...
                       guide = guide_colorsteps(barwidth = 25,
                                                ...
                                                title.vjust = 1,
                                                frame.colour = "black")) +
  scale_date(ini = 20181122060000, break_bin = "12 hours") +
...
imerg <- read_rds("/home/paola.corrales/mesoda/analysis/data/derived_data/IMERG_1h.rds")
...
pp_area %>% 
  ...
  scale_x_datetime(limits = c(ymd_h(2018112200), ymd_h(2018112312))) +
...
media %>% 
  ...
  scale_fill_manual(guide = NULL, values = colores_exp, labels = c(E2 = "CONV", E5 = "AUT", 
                                                                   E6 = "SATWND", E8 = "RAD")) +
  geom_line(aes(color = exp, linetype = estadistico)) +
  scale_color_manual(values = colores_exp, labels = c(E2 = "CONV", E5 = "AUT", 
                                                      E6 = "SATWND", E8 = "RAD")) +
...
pbl <- miembros[, .(RMSE = mean(RMSE),
                    tipo = "pbl"), 
                by = .(exp, fcst_exp, pbl, lev, variable, iop)] %>% 
  setnames("pbl", "param")

media %>% 
  .[estadistico == "RMSE"] %>% 
  .[, c("iop", "exp", "variable", "lev", "fcst_exp", "RMSE" = "value.est")] %>% 
  .[, ":="(param = "Mean",
           RMSE = value.est, 
           tipo = "Mean")] %>% 
  .[, value.est := NULL] %>% 
  ...
  # geom_hline(yintercept = 0, color = "grey30") +
  geom_line(aes(y = RMSE, color = exp, linetype = param, group = interaction(exp, param))) +
  scale_linetype_manual(values = c(1, 2, 3, 4)) +
  scale_color_manual(values = colores_exp, labels = c(E2 = "CONV", E5 = "AUT", 
                                                      E6 = "SATWND", E8 = "RAD")) +
  coord_flip() +
  facet_grid(factor(fcst_exp) +iop~variable, scales = "free_x", 
             labeller = labeller(variable = c("t" = "Temperature",
                                              "td" = "Dew point\ntemperature",
                                              "u" = "U wind",
                                              "v" = "V wind"))) +
...
cum <- miembros[, .(RMSE = mean(RMSE),
                    tipo = "cumulus"), 
                by = .(exp, fcst_exp, cumulus, lev, variable, iop)] %>% 
...
rbind(pbl, cum) %>% 
  .[, .(RMSE_mean = mean(RMSE, na.rm = TRUE)), 
    by = .(variable, exp, param, fcst_exp)] %>% 
  ggplot(aes(RMSE_mean)) +
  geom_histogram(aes(fill = exp), 
                 position = "identity", 
                 alpha = 0.5) +
   scale_fill_manual(values = colores_exp, 
...
rbind(pbl, cum) %>%
  .[, .(RMSE_mean = mean(RMSE, na.rm = TRUE)), 
    by = .(variable, exp, param, fcst_exp, tipo)] %>% 
  ggplot(aes(RMSE_mean, exp)) +
  ...
  scale_y_discrete(labels = c(E2 = "CONV", E5 = "AUT", 
                                 E6 = "SATWND", E8 = "RAD")) +
  facet_grid(fcst_exp ~variable, scales = "free_x") +
  labs(y = NULL, x = "mean RMSE", shape = NULL) +
  theme_minimal() +
...
media %>% 
  ...
  geom_line(aes(y = BIAS, color = exp, linetype = param, group = interaction(exp, param))) +
  geom_hline(yintercept = 0, color = "grey30") +
  scale_linetype_manual(values = c(1, 2, 3, 4)) +
  scale_color_manual(values = colores_exp, labels = c(E2 = "CONV", E5 = "AUT", 
                                                      E6 = "SATWND", E8 = "RAD")) +
...
rbind(pbl, cum) %>%
  ...
  scale_y_discrete(labels = c(E2 = "CONV", E5 = "AUT", 
                                 E6 = "SATWND", E8 = "RAD")) +
  facet_grid(fcst_exp ~ variable, scales = "free_x") +
  labs(y = NULL, x = "mean BIAS", shape = NULL) +
  theme_minimal() +
  theme(aspect.ratio = 1/2,
        panel.background = element_rect(fill = "grey98",
                                        color = "grey98"))
...
rbind(pbl, cum) %>%
  .[, .(BIAS_mean = mean(BIAS, na.rm = TRUE)), 
    by = .(var, exp, param, fcst_exp, tipo)] %>% 
  ggplot(aes(BIAS_mean, exp)) +
  geom_vline(xintercept = 0, color = "grey70") +
  geom_point(aes(color = exp, shape = param),
             size = 3) +
  ...
  scale_y_discrete(labels = c(E2 = "CONV", E5 = "AUT", 
                                 E6 = "SATWND", E8 = "RAD")) +
  facet_grid(fcst_exp ~ var, scales = "free_x") +
  labs(y = NULL, x = "mean BIAS", shape = NULL) +
  theme_minimal() +
  theme(aspect.ratio = 1/2,
        panel.background = element_rect(fill = "grey98",
...
```{r}
files <- Sys.glob(paste0(derived_data, "/ppacum/E[2,5,6,8]_fcst_201811220*_mean_acum_30h.rds"))
...
pp_acum %>% 
  ...
  geom_mapa() +
  facet_grid(fcst_init ~ exp, labeller = labeller(exp = c(E2 = "CONV", E5 = "AUT", 
                                                          E6 = "SATWND", E8 = "RAD"))) +
  labs(fill = NULL) +
  theme_minimal() +
  theme(legend.position = "bottom") +
...
```{r}
files <- Sys.glob(paste0(derived_data, "/ppacum/E[2,5,6,8]_fcst_201811220*_prop_acum_30h.rds"))
...
df <- data.frame(
  ...
  exp = rep(c("E2", "E5", "E6", "E8"), each = 3)
)

files <- purrr::map2_chr(df$lead_time, df$exp, function(l, e) {
  paste0("/home/paola.corrales/datosmunin3/EXP/", e, "/FCST/2018112200/NPP/NPP_2018-11-22_00_FC", 
...
var_00 <- purrr::map(files, function(f) {
  ...
  meta <- unglue(f, "/home/paola.corrales/datosmunin3/EXP/{exp}/FCST/{fcst_exp}/NPP/NPP_{x}_FC{lead}.nc")
...
files <- purrr::map2_chr(df$lead_time, df$exp, function(l, e) {
  paste0("/home/paola.corrales/datosmunin3/EXP/", e, "/FCST/2018112206/NPP/NPP_2018-11-22_06_FC", 
         formatC(l, width = 2, flag = "0"), ".nc")
})
...
var_00 %>% 
  ...
  geom_mapa() +
  cordillera +
  facet_grid(lead_date ~ exp, labeller = labeller(exp = c(E2 = "CONV", E5 = "AUT", 
                                                          E6 = "SATWND", E8 = "RAD"))) +
  theme_minimal() +
  theme(legend.position = "bottom",
        panel.ontop = TRUE,
...
colours <- c("#3D4170", "#3D4E8C", "#3B5B84", "#3A6695", "#3672A4", "#3188BD", 
