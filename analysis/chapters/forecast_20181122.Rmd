...
obs_ens_00 <- read_rds(paste0(derived_data, "/interp_obs/interp_obs_2018112200_mem.rds"))
obs_ens_06 <- read_rds(paste0(derived_data, "/interp_obs/interp_obs_2018112206_mem.rds"))
...
extremos <- rbind(obs_ens_00, obs_ens_06) %>% 
  .[, .(max_rmse = max(RMSE),
        min_rmse = min(RMSE)), by = .(var, exp, fcst_exp, date)]


miembros <- rbind(obs_ens_00, obs_ens_06)
