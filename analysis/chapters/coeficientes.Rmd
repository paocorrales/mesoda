...
```{r}
files <- list.files("/home/paola.corrales/datosmunin3/EXP/E7-training/ANA", 
...
#### Resumen para todos los sensores


```{r resume-bias, fig.cap= "Diferencias absoluta entre ambos experimentos para bias residual (color, igual en ambos paneles) y cantidad de observaciones que superan el control de calidad (tamaño de los puntos). En circulos negros se indentifican los sesnores y canales donde se asimilaron más observaciones comparando experimentos."}
files <- list.files("/home/paola.corrales/datosmunin/EXP/E7-training/ANA", 
                    pattern = "asim.", recursive = TRUE, 
                    full.names = TRUE)
files <- files[!str_detect(files, pattern = "conv")]
...
satinfo <- fread(here("analysis", "data", "raw_data", "satinfo.txt")) %>% 
  setnames(c("!sensor/instr/sat", "chan"), c("sensor", "channel"))


n_obs <- diag_rad[qc == 0, .N, by = .(sensor, channel, exp)] %>% 
  dcast(sensor + channel ~ exp) %>% 
  .[, win := fifelse(week2 > week1, "week2", "week1")] %>% 
  melt(measure.vars = c("week1", "week2"), variable.name = "exp", value.name = "N")
...
total <- diag_rad %>% 
  ...
  .[, abs_bias := abs(week2) - abs(week1)] %>% 
  melt(measure.vars = c("week1", "week2"), value.name = "bias_tbc", variable.name = "exp") %>% 
...
```{r caso1-prob, fig.cap="Probabilidad de precipitación acumulada > 5 mm (arriba) y > 25 mm (abajo) para cada experimento en el caso 1.", fig.height=3}
...
```{r caso2-prob, fig.cap="Probabilidad de precipitación acumulada > 5 mm (arriba) y > 25 mm (abajo) para cada experimento correspondiente al caso 2", fig.height=3}
pp[, .(prop = mean(precipitacion > 5)*100), by = .(exp, x, y)] %>% 
  ggplot(aes(x, y)) +
  geom_contour_fill(aes(z = prop), proj = norargentina_lambert,
                    breaks = seq(0, 100, 10)) +
