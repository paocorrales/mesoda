...
```{r n_rad, fig.cap="Cantidad de observaciones potencialmene asimilables en cada ciclo de asimilación para el experimento pre-RAD1"}
...
diag_rad %>% 
  ...
  .[sensor %in% c("airs", "iasi")] %>% 
  # .[, .N, by = .(date, channel, sensor)] %>% 
  ggplot(aes(channel, press)) +
  geom_boxplot(aes(group = factor(channel))) +
  facet_wrap(~sensor, scales = "free") +
  labs(x = "Canal", y = "Presión (hPa)") +
  theme_minimal() 
