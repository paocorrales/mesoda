...
```{r bias-temp-amsua, fig.cap="Bias medio de las radianzas de AMSU-A en las distintas plataformas y para los dos periodos a lo largo del tiempo.", fig.height=4}
...
```{r pred-amsua, fig.cap="Contribución a la corrección del bias de dos predictores para el canal 3 de MHS NOAA19 a las 8 UTC del 17/11/2018"}
...
pp <- purrr::map(files, function(f) {
  ...
  ReadNetCDF(f, vars = c("precipitacion" = "PP", 
                                "XLAT", 
                                "XLONG")) %>% 
    .[, .(precipitacion = mean(precipitacion, na.rm = TRUE)), by = .(XLAT, XLONG)] %>% 
...
pp %>% 
  dcast(XLAT + XLONG + x + y ~ exp, value.var = "precipitacion") %>% 
  .[, diff := week2 - week1] %>% 
  ggplot(aes(x, y)) +
