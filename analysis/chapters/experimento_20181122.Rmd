...
```
```{r}
files <- c(Sys.glob("/home/paola.corrales/datosmunin/EXP/E7/ANA/20181121010000/diagfiles*/asim_*ensmean"), Sys.glob("/home/paola.corrales/datosmunin/EXP/E7/ANA/20181121130000/diagfiles*/asim_*ensmean"))

files <- Sys.glob("/home/paola.corrales/datosmunin/EXP/E7/ANA/20181121130000/diagfiles*/asim_*ensmean")
...
rad %>% 
  ...
  .[, date := factor(date)] %>% 
  ggplot(aes(ConvertLongitude(lon), lat)) +
  geom_point(aes(color = sensor, shape = sensor), size = 0.8) +
  # scale_color_divergent(trans = mesoda::symlog_trans(),
  #                       guide = guide_colourbar(barheight = 15, barwidth = 1)) +
  scale_color_brewer(palette = "Dark2") +
  geom_mapa() +
...
dbz %>% 
  ...
  scale_color_dbz(aesthetics = "fill", super = ScaleDiscretised, 
                  guide = guide_colorsteps(barheight = 18)) +
  geom_mapa() +
  facet_grid(date~exp, labeller = labeller(exp = c(E4 = "CONV", E5 = "AUT", 
                                                   E6 = "SATWND", E7 = "RAD"))) +
...
sondeos %>% 
  ...
  .[, ":="(RMSE = mean(sqrt((value - fcst_value)^2)), 
           BIAS = mean(value - fcst_value)), by = .(iop, exp, variable, lev)] %>% 
