...
dbz %>% 
  ...
  geom_contour_fill(aes(z = max_dbz, fill = stat(level)), 
                    proj = norargentina_lambert,
                    breaks = values) +
  scale_fill_manual(values = colours, labels = function(x) JumpBy(x, 3, fill = ""),
                    drop = FALSE, guide = guide_colorsteps(barwidth = 30, 
                                                                         barheight = 0.8)) +
  
  # scale_color_dbz(name = NULL, aesthetics = "fill", 
  #                 guide = guide_colorstrip(barwidth = 20, 
  #                                          barheight = 0.8,
  #                                          inside = TRUE)) +
  # ggnewscale::new_scale_fill() +
  # geom_contour_fill(aes(z = max_dbz), fill = "white", 
  #                   proj = norargentina_lambert,
  #                   breaks = c(-Inf, 10)) +
  geom_mapa() +
