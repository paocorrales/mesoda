...
rad %>% 
  .[qc == 0 & iuse == 1] %>% 
  .[, lev := cut_width(peakwt, 50)] %>% 
  .[, .(omb = mean(tbc),
              n = .N), by = .(date, lev)] %>% 
  ggplot(aes(date, fct_rev(lev))) +
  geom_raster(aes(fill = omb)) +
  ...
  labs(x = NULL, fill = NULL, y = "pressure layer")
...
q[date == unique(date)[1:3] & west_east %between% c(70, 180)] %>% 
  ...
  geom_contour(aes(z = v_mean, linetype = factor(-sign(..level..))), 
               color = "grey30", size = 0.5, binwidth = 2) +
  geom_text_contour(aes(z = v_mean), color = "grey30", stroke = 0.1, breaks = seq(-10, 6, 2)) +
...
conv[type %in% c(181, 187) & exp %in% experiments, 
