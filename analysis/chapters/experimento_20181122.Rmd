...
```{r rad-wv}
files <- c(Sys.glob("/home/paola.corrales/datosmunin/EXP/E8/ANA/20181121020000/diagfiles*/asim_iasi*ensmean"), Sys.glob("/home/paola.corrales/datosmunin/EXP/E8/ANA/20181121140000/diagfiles*/asim_iasi_metop-b*ensmean"))
...
global_qv_E8 <- read_rds("~/mesoda/analysis/data/derived_data/omb_acum_qv_E8.rds")

rbind(global_qv_E6, global_qv_E7, global_qv_E8) %>%
  # .[exp == "E7"] %>% 
  .[, nivel := fcase(bottom_top %in% c(1:8), "lower",
                     bottom_top %in% c(9:13), "middle",
                     bottom_top %in% c(14:20), "upper",
                     bottom_top %in% c(21:38), "21 to 38")] %>%
  .[ bottom_top %between% c(1, 20)] %>% 
  .[, acum := cumsum(qv), by = .(bottom_top, exp)] %>%
  .[, acum := mean(acum), by = .(nivel, exp, date)] %>%
  # .[bottom_top %in% c(1:18)] %>%
  ggplot(aes(date, acum*1000)) +
  geom_line(aes(color = exp, linetype = exp, group = interaction(nivel, exp))) +
  ...
  geom_label(data = ~.x[date %in% unique(date)[67]],
             aes(label = nivel, color = exp, 
                 group = interaction(nivel, exp)),
             size = 3, fill = "white", show.legend = FALSE,
             label.size = NA, label.padding = unit(0.1, "lines")) +
  scale_date() +
  # scale_linetype_manual(values = c(2, 1, 3), labels = c(E4 = "CONV", E5 = "AUT",
  #                                                    E6 = "SATWND", E7 = "RAD", E8 = "NEW RAD")) +
  # scale_color_manual(values = c(colores_exp, "black"), labels = c(E4 = "CONV", E5 = "AUT",
  #                                                     E6 = "SATWND", E7 = "RAD", E8 = "NEW RAD")) +
...
```{r}
files <- Sys.glob("~/datosmunin/EXP/E8/ANA/2018112*/diagfiles/asim*.ensmean")
...
perfiles %>% 
  ...
  .[, ":="(E5_E2 = E5 - E2,
           ...
           E7_E6 = E7 - E6,
           E8_E6 = E8 - E6)] %>% 
  melt(measure.vars = c("E5_E2", "E6_E5", "E7_E6", "E8_E6")) %>% 
  ggplot(aes(date, lev)) +
...
files <- Sys.glob("/home/paola.corrales/datosmunin3/EXP/derived_data/perfiles_rmsd*")
...
perfiles_mse %>% 
  .[variable == "T"] %>% 
  ggplot(aes(date, lev)) +
  geom_contour_fill(aes(z = rmsd, fill = stat(level))) +
  scale_fill_viridis_c(option = "A", direction = -1,
                       name = NULL, super = ScaleDiscretised,
                       guide = guide_colorsteps(barwidth = 25,
                                                barheight = 0.8)) +
  labs(subtitle = "Temperature RMSD (K)",
       x = NULL,
       y = "Pressure level") +
...
perfiles %>% 
  ...
  scale_fill_divergent_discretised(name = NULL, limits = c(-1.6, 1.6),
                                   guide = guide_colorsteps(barwidth = 25,
                                                            barheight = 0.8)) +
...
perfiles_mse %>% 
  ...
  scale_fill_viridis_c(option = "A", direction = -1,
                       ...
                       guide = guide_colorsteps(barwidth = 25,
                                                barheight = 0.8)) +
  labs(subtitle = "Specific humidity RMSD (g/Kg)",
       x = NULL,
       y = "Pressure level") +
  scale_y_level(name = "Pressure level", breaks = c(1000, 850, 750, 500, 300, 200, 100)) +
  scale_x_datetime(expand = c(0,0), date_labels = "%HZ \n %b-%d", date_breaks = "24 hours") +
...
perfiles %>% 
  ...
  .[, ":="(E5_E4 = E5 - E4,
           ...
           E7_E6 = E7 - E6,
           E8_E6 = E8 - E6)] %>% 
  melt(measure.vars = c("E5_E4", "E6_E5", "E7_E6", "E8_E6")) %>% 
  ggplot(aes(date, lev)) +
  ...
  scale_fill_divergent_discretised(name = NULL,
                                   ...
                                   guide = guide_colorsteps(barwidth = 25,
                                                            barheight = 0.8)) +
  geom_contour2(data = ~.x[, .(lev, date, E5_E4 = E5, E6_E5 = E6, E7_E6 = E7, E8_E6 = E8)] %>%
                  melt(id.vars = c("lev", "date")) %>% unique(), aes(z = value),
                breaks = seq(0, 30, 3), size = 0.2, linetype = 1, color = "grey30") +
  geom_text_contour(data = ~.x[, .(lev, date, E5_E4 = E5, E6_E5 = E6, E7_E6 = E7, E8_E6 = E8)] %>%
                      melt(id.vars = c("lev", "date")) %>% unique(), aes(z = value),
  ...
  scale_date(ini = 20181121000000, break_bin = "12 hours") +
  facet_wrap(~variable, ncol = 2,
             labeller = labeller(variable = c("E5_E4" = "AUT - CONV",
                                                         "E6_E5" = "SATWND - AUT",
                                                         "E7_E6" = "RAD - SATWND",
                                                         "E8_E6" = "NEW RAD - SATWND"))) +
  theme_minimal() +
...
```{r U-mse, eval=FALSE, fig.cap="MSE", include=FALSE}
...
```{r V-mse, eval=FALSE, fig.cap="MSE", include=FALSE}
...
```{r summary, fig.height=10, fig.width=9}
...
files <- Sys.glob(paste0(derived_data, "mucape/mcape_ana_E*_20181122000000.nc")) 
files <- files[!str_detect(files, c("E3"))]
...
ana[bottom_top %between% c(2, 7), .(v_mean = mean(v)), 
  ...
  geom_contour2(aes(z = v_mean, size = stat(level)),
                proj = norargentina_lambert, color = "blueviolet", 
                breaks = c(-10, -5)) +
  scale_size_continuous(breaks = c(-10, -5), range = c(0.8, 0.4),
                        labels = c("-10 m/s", "-5 m/s"),
                        name = NULL) +
  ...
  geom_contour2(aes(z = cortante, size = stat(level)),
                ...
                breaks = c(15, 30)) +
  scale_size_continuous(breaks = c(15, 30), range = c(0.4, 0.8),
                        labels = c("15 m/s", "30 m/s"),
                        name = NULL) +
...
```{r}
files <- Sys.glob(paste0(derived_data, "mucape/mcape_ana_E[2,5,6,8]_20181122*.nc")) 

dates <- c("20181122000000", "20181122060000", 
           "20181122120000", "20181122180000",
           "20181123000000")
...
fcsts <- expand_grid(fcsts = dates,
                     exp = c("E2", "E5", "E6", "E8"))

cor <- lapply(seq_len(nrow(fcsts)), function(f) {
...
readRDS("/home/paola.corrales/mesoda/analysis/data/derived_data/IMERG_1h.rds") %>% 
  ...
  geom_contour2(aes(z = pp_acum, color = ..level..), 
                breaks = seq(1, 12, 1)) +
  scale_color_viridis_c(guide = NULL, breaks = seq(1, 12, 1), limits = c(1, 12)) +
  scale_date(ini = 20181121000000, break_bin = "12 hours") +
  ...
  geom_contour2(aes(z = pp_acum, color = ..level..),
                breaks = seq(1, 12, 1)) +
  scale_color_viridis_c(breaks = seq(1, 12, 1), limits = c(1, 12),
                        guide = guide_colorstrip(inside = TRUE,
                                                 barwidth = 25,
                                                 barheight = 0.8)) +
  scale_date(ini = 20181121000000, break_bin = "12 hours") +
...
files <- Sys.glob(paste0(derived_data, "dbz/maxdbz_ana_E[2568]_", dates, ".nc"))
...
dates <- rep(c("20181122110000"), 1, each = 5)
...
dbz %>% 
  ...
  geom_mapa() +
  facet_wrap(~exp, ncol = 3, labeller = labeller(exp = c(E4 = "CONV", E5 = "AUT", 
                                                         E6 = "SATWND", E7 = "RAD",
                                                         E8 = "NEW RAD"))) +
...
```{r dbz-ens}
files <- Sys.glob(paste0(derived_data, "dbz/maxdbz_ana_E[5,6,8]_0*_20181122130000.nc"))
...
dbz %>% 
  .[, .(max_dbz = mean(max_dbz, na.rm = TRUE)), by = .(exp, date, lon, lat, x, y)] %>% 
  # .[date %in% unique(date)[1:2]] %>% 
  .[, date := factor(date)] %>% 
  ggplot(aes(x, y)) +
  geom_contour_fill(aes(z = max_dbz, fill = stat(level)), 
...
fss %>% 
  ...
  geom_line(aes(color = exp)) +
  # geom_point(aes(color = exp)) +
  scale_color_manual(values = c("purple", colores_exp, "black"), labels = c(E4 = "CONV", E5 = "AUT", 
                                                      E6 = "SATWND", E7 = "RAD", E8 = "NEW RAD")) +
...
files <- Sys.glob("/home/paola.corrales/datosmunin/EXP/derived_data/sondeos/ANA/*")
