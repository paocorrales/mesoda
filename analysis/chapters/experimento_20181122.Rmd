...
rbindlist(list(no_oficiales, oficiales)) %>% 
  ...
  plot_layout(ncol = 3, widths = c(0.49, 0.02, 0.49))
```
...
```{r summary, fig.height=12, fig.width=10}
...
ana[bottom_top %between% c(1, 10), .(v_mean = mean(v)), 
  by = .(south_north, west_east, date, exp)] %>% 
  .[pw, on = .NATURAL] %>% 
ggplot(aes(x, y)) +
 # geom_point(aes(color = v_mean)) +
  geom_contour_fill(aes(z = value, fill = stat(level_d)),
                    proj = norargentina_lambert, alpha = 0.7) +
  scale_fill_distiller(name = NULL, super = ScaleDiscretised,
  ...
  geom_contour2(aes(z = v_mean, linetype = factor(-sign(v_mean))),
   ...
   geom_text_contour(aes(z = v_mean), size = 4, stroke = 0.05,
                     proj = norargentina_lambert, color = "darkorange", 
                breaks = c(-10, -5, -2, 0, 2, 5, 10),
...
dbz %>% 
  ...
  geom_contour_fill(aes(z = max_dbz, fill = stat(level_d)), 
                    ...
                    breaks = seq(0, 60, 5),) +
  scale_color_dbz(aesthetics = "fill", super = ScaleDiscretised, 
                  guide = guide_colorsteps(barheight = 18, barwidth = 1)) +
  geom_mapa() +
  facet_grid(cumulus~exp, labeller = labeller(exp = c(E4 = "CONV", E5 = "AUT", 
                                                   E6 = "SATWND", E7 = "RAD"))) +
  labs(x = NULL, y = NULL, fill = NULL) +
