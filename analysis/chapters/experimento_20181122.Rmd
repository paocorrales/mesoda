...
geom_mapa <- function(fill = NA) {
  list(geom_sf(data = map, fill = fill, color = "black", size = 0.2, inherit.aes = FALSE),
...
rbindlist(list(oficiales, no_oficiales, ascat, sondeos, aircraf, satwnd)) %>% 
...
rbindlist(list(no_oficiales, oficiales)) %>% 
  ...
  geom_point(alpha = 0.8, size = 1,
             aes(color = source)) +
  scale_color_manual(name =  NULL, values = c("Sfc - Non-official" = "#00695c", 
                                              "Sfc - Official" = "#FD8002"),
...
rad <- satinfo[rad, on = c("sensor", "channel")]
...
```
```{r rad-wv}
files <- c(Sys.glob("/home/paola.corrales/datosmunin/EXP/E7/ANA/20181121020000/diagfiles/asim_iasi*ensmean"), Sys.glob("/home/paola.corrales/datosmunin/EXP/E7/ANA/20181121140000/diagfiles/asim_iasi*ensmean"))
...
```{r T-mse, fig.cap="MSE Temperature between analysis and guess in Kelvin^2."}
...
```{r Q-mse, fig.cap="MSE Specific humidity between analysis and guess in g/Kg^2."}
...
```{r q-low, warning=FALSE, fig.align="center", message=FALSE}
...
```{r}
q[date == unique(date)[1:3] & west_east %between% c(70, 180)] %>% 
  .[, .(q_mean = mean(q),
  ...
  # geom_vector(aes(x = south_north, y = bottom_top,
  #                 mag = abs(v_mean), angle = fifelse(v_mean > 0, 0, 180)),
  #             arrow.angle = 6, size = 0.2, skip.x = 10) +
  # scale_mag() +
...
sondeos %>% 
  ...
  labs(color = "T", y = NULL, x = NULL) +
  theme_minimal() +
  theme(legend.position = "bottom") +
  
  sondeos %>% 
  .[variable == "t" & !str_detect(site, "/") & !is.na(fcst_value)] %>% 
...
```{r omb-q-evol, fig.cap="Evolution of the mean OmB for humidity throughout the periodod"}
...
```{r pbl-18Z, fig.cap="Zonal mean high of the PBL over the subdomain estimated for the PBL parameterization for Nov 21st, 18Z"}
...
readRDS("/home/paola.corrales/mesoda/analysis/data/derived_data/IMERG_1h.rds") %>% 
  ...
  .[end_date %between% c(as_datetime("20181120190000"), as_datetime("20181123120000"))] %>% 
    .[, .(pp_acum = mean(pp_acum)), by = .(end_date, y)] %>% 
  ggplot(aes(end_date, y/100000)) +
  geom_contour(aes(z = pp_acum, color = ..level..), 
               breaks = seq(0, 12, 0.5)) +
  scale_color_viridis_c(guide = NULL, breaks = seq(0, 12, 0.5)) +
  scale_date(ini = 20181121000000, break_bin = "12 hours") +
  labs(x = NULL, y = NULL, color = "PP acum", 
       subtitle = "IMERG") +
  theme_minimal() +
  theme(legend.position = "bottom") +

  ppacum_1h %>% 
  .[, .(pp_acum = mean(pp_acum)), by = .(exp, date, y)] %>% 
  ggplot(aes(date, y/100000)) +
  geom_contour(aes(z = pp_acum, color = ..level..), 
               breaks = seq(0, 12, 0.5)) +
  scale_color_viridis_c(breaks = seq(0, 12, 1), limits = c(0, 12),
                        guide = guide_colorstrip(barwidth = 25,
                                                 barheight = 1)) +
  scale_date(ini = 20181121000000, break_bin = "12 hours") +
  facet_wrap(~ exp, labeller = labeller(exp = c(E4 = "CONV", E5 = "AUT", 
                                                E6 = "SATWND", E7 = "RAD"))) +
...
.[, .(prop = mean(prop)), by = .(exp, date, y, umbral)] %>% 
  .[umbral %in% c(1, 5)] %>% 
ggplot(aes(date, y)) +
  geom_contour_fill(aes(z = prop*100, fill = stat(level)),
                    breaks = seq(0, 100, 10)) +
  scale_color_gradientn(colours = rev(Redmonder::redmonder.pal(10, name = "qMSOStd")), 
                       limits = c(10, 100),
                       na.value = "white",
...
```{r}
files <- Sys.glob("/home/paola.corrales/mesoda/analysis/data/derived_data/fss_6h_ana_E[4-7]new.csv")
