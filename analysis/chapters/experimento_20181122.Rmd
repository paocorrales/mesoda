...
```{r mapa-obs, fig.cap="Localization of meteorological stations and ships for official prep-BUFR (orange dots) and automatic statios (green dots)."}
...
rbindlist(list(no_oficiales, oficiales)) %>% 
  ...
  scale_color_manual(values = c("darkorange", "purple", "cyan4", "grey70"),
                     guide = guide_legend(override.aes = list(size = 2))) +
  scale_size_manual(values = c(0.7, 0.5, 1.5, 0.1)) +
  # scale_color_brewer(palette = "Dark2") +
  # khroma::scale_color_vibrant() +
  geom_point(data = square, aes(lon, lat), size = 0.2) +
  scale_x_continuous(expand = c(0, 0)) +
  scale_y_continuous(expand = c(0, 0)) +
  labs(x = NULL, y = NULL, color = NULL, shape = NULL, size = NULL) +
  theme_minimal() +
  theme(legend.position = "bottom") +
  
  plot_layout(ncol = 3, widths = c(0.47, 0.06, 0.47))
```
...
dbz %>% 
  ...
  scale_color_dbz(aesthetics = "fill", super = ScaleDiscretised, 
                  guide = guide_colorsteps(barheight = 18)) +
  geom_mapa() +
  facet_grid(cumulus~exp, labeller = labeller(exp = c(E4 = "CONV", E5 = "AUT", 
                                                   E6 = "SATWND", E7 = "RAD"))) +
  labs(x = NULL, y = NULL, fill = NULL) +
  theme_minimal() +
