...
files_ana <- Sys.glob("~/datosmunin3/EXP/prueba_abi/ANA/20181121000000_*/analysis.mem001")

ana <- map(files_ana, function(f) {
  
  meta <- unglue(f, "/home/paola.corrales/datosmunin3/EXP/prueba_abi/ANA/20181121000000_{exp}/analysis.mem001")
  
  ReadNetCDF("~/datosmunin3/EXP/prueba_abi/ANA/20181120180000/analysis.mem001", 
                  vars = c(T_ana = "T", lon = "XLONG", lat = "XLAT")) %>% 
...
gue <- ReadNetCDF("~/datosmunin3/EXP/prueba_abi/GUESS/20181121000000/wrfarw.mem001", 
                  vars = c(T_guess = "T", lon = "XLONG", lat = "XLAT")) %>% 
  .[, c("x", "y") := wrf_project(lon, lat)] 
...
ana[gue, on = .NATURAL] %>% 
  .[, ":="(level = fcase(bottom_top <= 10, "PBL",
    ...
    .[, a_g := T_ana - T_guess] %>%
  .[level %in% c("low", "middle", "upper") & run %in% c("all", "ch08")] %>% 
  ggplot(aes(x, y)) +
...
ana[gue, on = .NATURAL] %>% 
  .[, ":="(level = fcase(bottom_top <= 10, "PBL",
    ...
    .[, a_g := T_ana - T_guess] %>%
  .[bottom_top == 1] %>% 
  dcast(x + y ~ run, value.var = "a_g") %>% 
  ggplot(aes(x, y)) +
  geom_contour_fill(aes(z = all - noabi, fill = ..level..), proj = norargentina_lambert) +
  scale_fill_divergent_discretised() +
  geom_mapa() 
...
ana <- ReadNetCDF("~/datosmunin3/EXP/prueba_abi/ANA/20181120180000/analysis.mem001", 
  ...
  .[, c("x", "y") := wrf_project(lon, lat)] 
gue <- ReadNetCDF("~/datosmunin3/EXP/prueba_abi/ANA/analysis.mem001",
